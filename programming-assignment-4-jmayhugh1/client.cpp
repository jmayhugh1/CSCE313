...
void worker_thread_function(BoundedBuffer *response_buffer, BoundedBuffer *request_buffer, TCPRequestChannel *channel, int m)
{
...
int main(int argc, char *argv[])
{
    ...
    string a = "127.0.0.1";        // ip address of the server
    string r = "12000";           // port number of the server
    string f = "";       // name of file to be transferred
    cout << "about to read all the args" << endl;
    // read arguments
    int opt;
    ...
    //declare a vector of pointers to TCPRequestChannels
    ...
    // a new TCPRequestChannel is created for each worker thread
for (int i = 0; i < w; i++){
      
        TCPRequestChannel *chan = new TCPRequestChannel(a,r); // create new channel
        worker_message[i] = thread(worker_thread_function, &response_buffer, &request_buffer, chan, m);
